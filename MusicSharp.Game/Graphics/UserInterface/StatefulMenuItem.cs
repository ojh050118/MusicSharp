...
{
    public abstract class StatefulMenuItem : MusicSharpMenuItem
    {
        ...
        protected StatefulMenuItem(string text, Func<object, object> changeStateFunc, MenuItemType type, Action<object> action, IconUsage icon = default)
            : base(text, icon, type)
        {
            Action.Value = () =>
            {
